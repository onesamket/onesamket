  ...
  return (
    <section className="border-t-2 border-[#0759bd] pt-12">
      <div className="mb-9 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-[11px] font-extrabold uppercase tracking-[0.24em] text-[#0759bd]">Selected Work</p>
          <h2 className="mt-2 text-4xl font-black leading-none text-[#0759bd] md:text-5xl">Projects</h2>
        </div>
        <span className="text-sm font-bold uppercase tracking-[0.18em] text-[#527db4]">{allProjects.length} total</span>
      </div>
...
const ProjectRow: React.FC<{ title: string; description: string; link: string; index: number }> = ({ title, description, link, index }) => {
  ...
  return (
    <li className="grid gap-4 border-b border-[#0759bd]/25 py-5 md:grid-cols-[56px_minmax(0,1fr)_40px] md:items-start">
      ...
      <div>
        <h3 className="text-xl font-extrabold leading-tight text-[#073f8f]">{title}</h3>
